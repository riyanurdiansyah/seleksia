...
export default function ForceResetPage() {
    ...
    const handleReset = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError("Network error. Silakan periksa koneksi internet Anda.");
            setLoading(false);
        }
    };
            ...
            <div className="hidden lg:flex lg:w-[55%] bg-gradient-to-br from-[#1A3C40] via-[#0c5c64] to-[#059669] relative overflow-hidden flex-col justify-between p-16">
                ...
                <div className="relative z-10 max-w-xl my-auto space-y-8">
                    <div className="inline-flex items-center gap-2 bg-white/10 text-white/90 px-4 py-2 rounded-full text-xs font-bold border border-white/20 backdrop-blur-sm">
                        <span className="material-symbols-outlined text-xs text-[var(--color-warning)]">security</span>
                        <span>Keamanan Data Prioritas Kami</span>
                    </div>

                    <h2 className="text-5xl font-extrabold leading-[1.15] text-white tracking-tight">
                        Wajib Ubah<br />
                        <span className="bg-gradient-to-r from-[var(--color-warning)] to-white bg-clip-text text-transparent">Kata Sandi Anda</span><br />
                    </h2>

                    <p className="text-white/80 text-base leading-relaxed max-w-lg">
                        Untuk alasan keamanan, sistem mendeteksi bahwa Anda masih menggunakan kata sandi bawaan. Silakan perbarui kata sandi Anda untuk melanjutkan akses ke dalam sistem.
                    </p>
                </div>
